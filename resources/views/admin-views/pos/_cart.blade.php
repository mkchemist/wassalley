    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <?php
                    ...
                    $minimumProductQuantity = $cartItem['unit']['quantity'];
                    ?>
            <tr>
                ...
                <td class="align-items-center text-center">
                    <input type="number" data-key="{{$key}}" style="width:50px;text-align: center;"
                        value="{{$cartItem['quantity']}}"  onkeyup="updateQuantity(event)"
                        min="{{ $minimumProductQuantity }}" step="{{ $minimumProductQuantity }}">
                </td>
...
<div class="box p-3">
    <dl class="row text-sm-right">
        ...
        <dd class="col-sm-6 text-right"><button class="btn btn-sm" type="button" data-toggle="modal"
            ...
            \App\CentralLogics\Helpers::set_symbol($extra_discount) }}</dd>
        ...
        <dd class="col-sm-6 text-right h4 b">{{ \App\CentralLogics\Helpers::set_symbol(round($total+$total_tax, 2)) }}
        </dd>
    </dl>
    <div class="row">
                ...
                <form action="{{route('admin.pos.order')}}" id='order_place' method="post" class="row">
                    ...
                    <div class="form-group col-12">
                        <label class="input-label"
                            for="">{{translate('amount')}}({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
                        <input type="number" class="form-control" name="amount" min="0" step="0.01"
                            value="{{ Helpers::set_price(round($total+$total_tax, 2)) }}" disabled>
                    </div>
                    <div class="form-group col-12">
