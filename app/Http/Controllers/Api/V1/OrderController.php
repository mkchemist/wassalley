...
use App\Services\Payments\PointPayments\Exceptions\InsufficientPointPaymentException;
use App\Services\Payments\PointPayments\PointPayments;
use Carbon\Carbon;
...
class OrderController extends Controller
{
  ...
  public function place_order(Request $request)
  {
     ...
     */
    $user = $request->user();
    $pointPerCurrency =  Helpers::get_business_settings('point_per_currency');
    $amount = $request->order_amount;

    try {
      DB::beginTransaction();
      if ($request->payment_method === "points") {
        if (!PointPayments::hasSufficientPoints($user->point, $pointPerCurrency, $amount)) {
          $points = PointPayments::getOrderAmountEqualPoints($amount, $pointPerCurrency);
          throw new InsufficientPointPaymentException(
            "Insufficient point you must have at least $points"
          );
        }
      }
      ...
      PointPayments::handlePointWithdrawal($user, $amount, $pointPerCurrency, $o_id);
      DB::commit();
      return response()->json([
        'message' => translate('order_success'),
        'order_id' => $o_id
      ], 200);
    } catch (InsufficientPointPaymentException $e) {
      return response()->json([
        'message' => $e->getMessage(),
        "value" => PointPayments::getOrderAmountEqualPoints($amount, $pointPerCurrency)
      ], 400);
    } catch (\Exception $e) {
      report($e);
      DB::rollBack();
